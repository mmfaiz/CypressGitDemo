...
/*
  ...
  // select indoor game
    ...
    cy.get('select').eq(1).select('5571').should('have.value', '5571')
    cy.get('#btnSubmit').click() // submit payment
    cy.get('#userBookingModal > .modal-dialog > .modal-content > .modal-footer > .btn').click() // finish booking
  ...
  // select indoor game
    ...
    cy.get(':nth-child(10) > .row > .toggle-collapse-chevron > .right-margin10').click()
...
cy.get('.navbar-brand').contains('matchi',{ matchCase: false }) // check Matchi text button
      ...
      .get('.navbar-right > .user-menu > .dropdown-menu > :nth-child(2) > a').click()
