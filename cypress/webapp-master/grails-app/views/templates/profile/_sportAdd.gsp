    ...
    <div class="modal-content">
        <div class="modal-header">
            ...
            <h4 class="modal-title" id="addSportModalLabel">
                <g:message code="templates.profile.sportAdd.message1"/> <strong>${sport}</strong>
            </h4>
        </div>

        <g:form action="addSport" name="addSport" class="form-horizontal" role="form">
            <div class="modal-body">
                    ...
                    <g:each in="${SportProfileMindset.list()}" var="mindSet" status="i">
                        <div class="col-sm-4">
                            <div class="checkbox checkbox-info checkbox-inline">
                                <g:checkBox id="${mindSet}" name="mindset" value="${mindSet}" checked="${profile?.mindSet != null}"/>
                                <label for="${mindSet}">
                ...
                <select id="frequency" name="frequency" data-style="form-control">
                    <g:each in="${SportProfile.Frequency.list()}">
                        <option value="${it}" ${profile?.frequency == it ? "selected":""}><g:message code="sportprofile.frequency.${it}"/></option>
                    </g:each>
                </select>
            ...
            <div class="modal-footer">
                ...
                <g:submitButton name="submit" class="btn btn-md btn-success" value="${message(code: 'button.add.label')}"/>
            </div>
        </g:form>
    </div>
</div>
