...
<p class="lead header">
    <span class="${active?.size() < 1?"transparent-60":""}">
        <g:message code="offer.label"/> ${active?.size() > 0 ? " - ${active?.size()}${message(code: 'unit.st')}" : ""}
    </span>
    <g:if test="${active?.size() > 4}">
        <g:remoteLink controller="facilityCustomer" action="showCoupons" update="customerModal" class="btn btn-small"
                      style="vertical-align: text-bottom;" title="${message(code: 'templates.customer.customerCoupon.message12')}"
                      onFailure="handleAjaxError()" onSuccess="showLayer('customerModal')"
                      params="[ 'couponIds': active?.collect { it.id }, 'customerId':customer.id ]"><g:message code="default.multiselect.checkAllText"/></g:remoteLink>
    </g:if>

    <g:if test="${!customer.archived}">
        <g:remoteLink class="pull-right" controller="facilityOffer" action="addCouponForm" update="customerModal"
                      onFailure="handleAjaxError()" onSuccess="showLayer('customerModal')"
    ...
    <g:if test="${active.size() < 1}">
        <br>
        <small class="empty"><g:message code="templates.customer.customerCoupon.message2"/></small>
    </g:if>
</p>
<g:if test="${active?.size() > 0 || locked?.size() > 0 || archive.size() > 0}">
    <table class="table table-transparent table-condensed table-noborder table-striped table-fixed no-bottom-margin">
        ...
        <th class="center-text" style="width: 22%"><g:message code="templates.customer.customerCoupon.message5"/></th>
        ...
        <g:if test="${activeCoupons}">
            ...
            <g:each in="${activeCoupons}" var="coupon" status="i">
                <tr class="${i > 3 ? "hidden" : ""}">
                    ...
                    <td class="center-text">
                        <g:remoteLink class="pull-right" controller="facilityOffer" action="editCouponForm"
                                      update="customerModal"
                                      onFailure="handleAjaxError()" onSuccess="showLayer('customerModal')"
                                      params="['id': coupon.id, customerId: customer.id]"><i
                                class="icon-edit"></i></g:remoteLink>
                    </td>
                    <td class="center-text">
        ...
        <g:if test="${locked?.size() > 0}">
            <tr>
                <td class="lockedControl" onclick="toggleLocked()" style="cursor: pointer;" colspan="6">
                    <em><g:message code="templates.customer.customerCoupon.message13" args="[locked?.size()]"/> <i id="locked-indicator" class="icon-chevron-right"></i></em>
                </td>
            </tr>
            <g:each in="${locked}" var="lockedCoupon">
                <tr class="lockedCoupon" style="display: none;">
                    <td class="ellipsis">${lockedCoupon.coupon.name}</td>
                    <td class="center-text">${lockedCoupon.coupon.unlimited ? "~" : lockedCoupon.nrOfTickets} <g:message code="unit.st"/></td>
                    <td class="center-text">${lockedCoupon.expireDate ? lockedCoupon.expireDate.toString("${message(code: 'date.format.dateOnly')}") : ""}</td>
                    <td class="center-text">
                        <span class="label label-important"><g:message code="templates.customer.customerCoupon.message8"/></span>
                    </td>
                    <td class="center-text">
                        <g:remoteLink class="pull-right" controller="facilityOffer" action="editCouponForm" update="customerModal"
                                      onFailure="handleAjaxError()" onSuccess="showLayer('customerModal')"
                                      params="[ 'id': lockedCoupon.id, customerId: customer.id ]"><i class="icon-edit"></i></g:remoteLink>
                    </td>
                    <td class="center-text">
                        <g:if test="${!lockedCoupon.coupon.unlimited}">
                            <g:remoteLink controller="facilityOffer" action="showUsageHistory" id="${lockedCoupon.id}"
                                    update="customerModal" onFailure="handleAjaxError()" onSuccess="showLayer('customerModal')"
                                    title="${message(code: 'customerCoupon.usageHistory.tooltip')}" rel="tooltip"
                                    ><i class="icon-search"></i></g:remoteLink>
                            </g:if>
        ...
        <g:if test="${archive?.size() > 0}">
            <tr>
                <td class="archiveControl" onclick="toggleArchive()" style="cursor: pointer;" colspan="6">
                    <em><g:message code="templates.customer.customerCoupon.message14" args="[archive?.size()]"/> <i id="archive-indicator" class="icon-chevron-right"></i></em>
                </td>
            </tr>
            <g:each in="${archive}" var="usedCoupon">
                <tr class="usedCoupon" style="display: none;">
                    ...
                    <td class="center-text" width="100">${usedCoupon.expireDate ? usedCoupon.expireDate.toString("${message(code: 'date.format.dateOnly')}") : ""}</td>
                    <td class="center-text">
                        <span class="label label-info"><g:message code="templates.customer.customerCoupon.message10"/></span>
                    </td>
                    <td class="center-text">&nbsp;</td>
                    <td class="center-text">
                        <g:if test="${!usedCoupon.coupon.unlimited}">
                            <g:remoteLink controller="facilityOffer" action="showUsageHistory" id="${usedCoupon.id}"
                                    update="customerModal" onFailure="handleAjaxError()" onSuccess="showLayer('customerModal')"
                                    title="${message(code: 'customerCoupon.usageHistory.tooltip')}" rel="tooltip"
                                    ><i class="icon-search"></i></g:remoteLink>
                            </g:if>
