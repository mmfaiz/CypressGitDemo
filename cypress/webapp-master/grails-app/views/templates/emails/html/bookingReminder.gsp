                                    ...
                                    <table cellspacing="10" width="100%" style="width: 100%;">
                                        <tr>
                                            <td style="font-weight: normal;font-size: 14px;line-height: 1.6;">
                                                ...
                                                <g:message code="templates.emails.html.bookingReminder.message1" args="[
                                                        formatDate(date: slots[0].startTime, format: 'yyyy-MM-dd HH:mm'),
                                                        formatDate(date: slots[0].endTime, format: 'HH:mm'),
                                                        slots.court.name.join(', '),
                                                        customer?.facility?.name?.encodeAsHTML()]"/>:<br><br>

                                                <g:each in="${links}" var="link">
                                                    <a href="${link}" target="_blank"><g:message code="button.unbook.label"/></a><br/>
                                                </g:each>
                                                <br/>

                                                <g:if test="${accessCode}">
                                                    <g:message code="templates.emails.accessCode"/>: <strong>${accessCode}</strong>
                                                    <br/><br/>
                                                </g:if>


                                                <g:message code="templates.emails.html.bookingReminder.message2"/>
                                                </p>
                                                <!-- /Welcome ingress -->

                                                <!-- Callout Panel -->
                                                <g:if test="${facilityText}">
                                                    <table cellpadding="10" bgcolor="#e4eed6" style="background-color: #e4eed6;width: 100%;" width="100%">
                                                        <tr>
                                                            <td style="font-weight: normal;font-size: 14px;line-height: 1.6;">
                                                                <g:toRichHTML text="${facilityText?.toString()}" />
                                                            </td>
                                                        </tr>
                                                    </table>
                                                </g:if>
