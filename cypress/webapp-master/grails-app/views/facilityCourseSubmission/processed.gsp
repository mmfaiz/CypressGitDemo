<%@ page import="com.matchi.activities.Participant; com.matchi.dynamicforms.Submission; com.matchi.Customer; com.matchi.excel.ExcelExportManager; com.matchi.facility.FilterSubmissionCommand; com.matchi.FacilityProperty; com.matchi.courses.EditParticipantService" %>
<g:set var="returnUrl" value="${createLink(absolute: true, action: 'processed', params: params.subMap(params.keySet() - 'error' - 'message'))}"/>
<!doctype html>
<html>
<head>
    ...
    <r:script>
        ...
        $("#submissionStatus").allselectpicker({
            selectAllText: "${message(code: 'default.multiselect.checkAllText')}",
            countSelectedText: "{0} ${message(code: 'facilityCourseParticipant.index.memberStatuses.selectedText')}",
            selectedTextFormat: 'count'
        });
    ...
    <g:if test="${!facility?.isMasterFacility()}">
        <form method="GET" class="form well no-bottom-padding">
            <div class="row">
                ...
                <div class="form-group col-sm-2 no-top-margin">
                    <g:select from="${[Submission.Status.ACCEPTED, Submission.Status.DISCARDED]}" name="submissionStatus" value="${cmd?.submissionStatus}"
                              valueMessagePrefix="courseSubmission.submission.status" multiple="multiple"
                              title="${message(code: 'facilityCourseSubmission.processed.submissionStatus.noneSelectedText')}"/>
                </div>
        ...
        <div class="panel panel-default panel-admin">
            ...
            <g:form name="submissions" class="no-margin">
                ...
                <g:each in="${cmd?.submissionStatus}">
                    <g:hiddenField name="submissionStatus" value="${it}"/>
                </g:each>
                ...
                <g:hiddenField name="processed" value="${Boolean.TRUE}"/>
                ...
                <table id="submission-table" class="table table-striped table-hover table-bordered">
                    ...
                    <tr>
                        ...
                        <g:sortableColumn property="c.firstname" params="${params}" class="vertical-padding10" title="${message(code: 'courseParticipant.customer.label')}"/>
                        ...
                        <g:sortableColumn property="status" titleKey="courseSubmission.submission.status.label" params="${params}" defaultOrder="asc"/>
                        ...
                        <tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
                            ...
                            <td>
                                <g:link controller="facilityCustomer" action="show" id="${submissionInstance.customer.id}">
                                    ${submissionInstance.customer.fullName()}
                                </g:link>
                            </td>
                            <td>
                            ...
                            <td>
                                <span class="label ${message(code: "courseSubmission.cssClass.${submissionInstance.status}")}">
                                    ${message(code: "courseSubmission.submission.status.${submissionInstance.status}")}
                                </span>
                            </td>
                            <td class="text-center rowlink-skip">
        ...
        <g:if test="${totalCount > paginationStepSize}">
            <div class="text-center">
                <g:b3PaginateTwitterBootstrap prev="&laquo;" next="&raquo;" class="pagination-centered"
                                              maxsteps="0" max="${paginationStepSize}" action="processed" total="${totalCount}" params="${params}"/>
            </div>
        </g:if>
    </g:if>
